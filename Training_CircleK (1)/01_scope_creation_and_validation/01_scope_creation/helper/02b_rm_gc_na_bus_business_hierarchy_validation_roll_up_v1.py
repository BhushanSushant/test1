...
# Date Updated: This notebook is a clone of Accentures notebook with a similar name. This maps product keys from an older enviroment to the new enviroment. We are trying to replace this process with a static table ( work under progress ) . This notebook can be used for RM/GC run the other notebook
...
# """).createOrReplaceTempView("na_all_bu_transaction_full_table_hierarchy_validation_{0}".format(wave.lower()))
...
# MAGIC %md
# MAGIC #### View of the Main Table

# COMMAND ----------

# display(sqlContext.sql(""" 
#                         SELECT * from na_all_bu_transaction_full_table_hierarchy_validation_{0}
#                         LIMIT 5 
#                         """.format(wave.lower())))
...
# SELECT *
# FROM {0}
# WHERE CONCAT(transaction_type_key, "||", sys_environment_name) IN {1}
# AND category_desc IN (SELECT distinct category_desc from localized_pricing.acc_cat WHERE Acc_cat != 'Out of Scope')
# """.format(merged_table_df,transaction_type_key_ENV))
...
data_mb_agg = data_mb_roll_up_condition.filter(~((col("item_description").like('%OPEN KEY%')) | (col("item_description").like("%open key%"))))
...
for category in final_cat_list:
  # Subset data
  tmp_data = data_mb_agg.filter(col("category_desc")==category).select("marketbasket_header_key", "product_key", "sum_of_sales_amount", "sum_of_quantity_sold")
  
  
  # Capture quantile metrics - upper
  tmp_category_metrics_upper = tmp_data.approxQuantile(["sum_of_sales_amount","sum_of_quantity_sold"],[upper_bound],0)
  # Capture quantile metrics - lower
  tmp_category_metrics_lower = tmp_data.approxQuantile(["sum_of_sales_amount","sum_of_quantity_sold"],[lower_bound],0)
  ...
  tmp_remove_df_upper = tmp_data.filter(col("sum_of_quantity_sold")>tmp_qty_cutoff_upper)
  # Filter data down to records above cutoff - lower
  tmp_remove_df_lower = tmp_data.filter(col("sum_of_quantity_sold")<tmp_qty_cutoff_lower)
...
2869577151.7106614-2867518619.781631-1799027.2400000237-259504.64999997473
