...
# MAGIC 
# MAGIC Updates: added code to fetch data quality flags for all items from 02c scoping analysis helper, added code for identification of primary items
# MAGIC 
# MAGIC ##### Created By: Anson Park, Darren Wang
# MAGIC ##### Updated By: Jagruti Joshi, Anuj Rewale, Kushal Kapadia, Sai Kommisetty, Jing Lou, Binlun Feng
# MAGIC ##### Last Update on 2022-05-03
...
FILE_DIR = "/dbfs/Phase3_extensions/Elasticity_Modelling/Apr22_Test_Refresh/GR_Repo/Input/gr_initial_scope_Apr22_test_refresh_dummy.xlsx" 
#Testing IE
##FILE_DIR = "/dbfs/Phase3_extensions/Elasticity_Modelling/Apr22_Test_Refresh/IE_Repo/Input/ie_final_scope_Apr22_Test_Refresh.xlsx" 
...
##df = pd.read_excel(FILE_DIR, sheet_name='Final Scope', keep_default_na=False)

# COMMAND ----------

# MAGIC %md
# MAGIC #### PRIMARY ITEM IDENTFICATION 

# COMMAND ----------

# get the data quality flags from module 2 02c scoping analysis helper output for all items
...
df_temp = df.loc[:,~df.columns.isin(['latest_date_diff_flag','consecutive_flag','discont_flag','data_enough_flag'])]
df = df_temp.merge(scope_flags, how="left", on="upc")

# COMMAND ----------

# MAGIC %md
# MAGIC ##### Guidelines for primary item identification
...
# MAGIC ###### if # of flagged price families is low, recommend BU to remove them 
# MAGIC ###### if # of flagged price families is high OR BU doesn't want to remove flagged price families, the DS team can either relax the thresholds in the 02c scoping analysis helper and re-run the code before sending out the file to the BU OR can potentially use flagged items/ items with as primary if that item has the least critical data quality flag or that item is in top 80% sales

# COMMAND ----------

#df['primary/secondary'] = np.where(df['final_recommendation']== 'goodstuff', 'primary', 'secondary')
df['scope'] = np.where(df['data_quality_flag']== 'goodstuff' || df['cum_sales_flag']== 'within top 80.0% of sales', 'primary', 'secondary')
...
#convert 'sales_amount' column to float after adding in 0s for all blanks
df.loc[df['sales_amount'].isin(['']), 'sales_amount'] = '0'
...
#combine 'scope' and 'new_scope' columns into 'scope_final' column 
...
#category_revenue_group = df.loc[df['sales_amount'] != 0].groupby('category')['sales_amount'].agg(['sum', 'count'])  #to exclude zero revenue items
category_revenue_group = df.groupby('category')['sales_amount'].agg(['sum', 'count'])
...
if not df_dropped.empty:
  ...
  #flag dropped items that have sales_amount greater than revenue_lim - revenue lim in parentheses
  df_dropped.loc[df_dropped['sales_amount'] >= df_dropped['revenue_lim'], 'high_revenue_flag'] = 1
...
if standalone_secondary_items.shape[0] > 0:
  print(f'{standalone_secondary_items.shape[0]} items are standalone secondary items')
else:
