...
# MAGIC %python
# MAGIC bu_codes = ["FL" ,"CC", "SE" ,"RM" ,"GC" ,"WC", "TX", "SA" ,"GR", "NT", "MW" ,"GL" ,"HLD" ,"QW", "QE", "CE", "WD"]
# MAGIC dbutils.widgets.dropdown("bu_code", "GL", bu_codes)
...
# MAGIC library(lubridate)
# MAGIC options(scipen = 999)
# MAGIC Business_Units = c("1400 - Florida Division","1600 - Coastal Carolina Division", "1700 - Southeast Division","1800 - Rocky Mountain Division","1900 - Gulf Coast Division", "2600 - West Coast Division", "2800 - Texas Division",  "2900 - South Atlantic Division", "3100 - Grand Canyon Division",  "3800 - Northern Tier Division", "4100 - Midwest Division","4200 - Great Lakes Division","4300 - Heartland Division", "QUEBEC OUEST", "QUEBEC EST - ATLANTIQUE", "Central Division",  "Western Division")
# MAGIC BU_abbr =tolower(c("FL" ,"CC", "SE" ,"RM" ,"GC" ,"WC", "TX", "SA" ,"GR", "NT", "MW" ,"GL" ,"HLD" ,"QW", "QE", "CE", "WC"))
# MAGIC 
# MAGIC bu_df =data.frame(Business_Units,BU_abbr)
# MAGIC bu_abb = bu_df %>% filter(Business_Units %in% dbutils.widgets.get("business_unit")) %>% select(BU_abbr)
# MAGIC bu_abb = bu_abb[1,1]
# MAGIC bu <- paste(toupper(bu_abb), "_Repo", sep="")  # GC_repo #SE_repo #TC: automate based on short forms
# MAGIC 
# MAGIC #other inputs
# MAGIC 
# MAGIC wave <- dbutils.widgets.get("wave")
# MAGIC pre_wave <- '/dbfs/Phase4_extensions/Elasticity_Modelling/'
...
tmp = reduced_ads.merge(product_map_df, how='left', left_on='itemid', right_on='product_key')
display(tmp[tmp.category_desc.isin(['005-WINE','006-LIQUOR','004-BEER','085-SOFT DRINKS'])])


# COMMAND ----------

# MAGIC %r
# MAGIC ads_file_70 = reduced_ads%>%group_by(itemid, Cluster) %>% summarise(revenue = sum(revenue))
# MAGIC display(ads_file_70)
